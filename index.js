// index.js — Vivaho WhatsApp Matrimony Bot (Simplified)
...
const PAYMENT_REQUIRED_MSG = `❌ Please make payment to send interest and view contact details.

💰 Plans:
• ₹300 for 3 months (Send Interest)
• ₹1000 for 1 year (Send Interest)
• ₹2000 for 1 year (Interest + Contact)

Type *JOIN* to make payment or *SEARCH* to continue browsing.`;
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // SEARCH BRIDE/GROOM with loading message (FIXED)
    if (effectiveInput === "SEARCH_BRIDE" || effectiveInput === "SEARCH_GROOM") {
      ...
      const results = applyFiltersToVisibleProfiles(allVisible, { targetGender, excludeProfileId: null });
      if (!results.length) {
    ...
    if (effectiveInput === "FILTER_SEARCH") {
      await sendText(from, "Filter search coming soon. Use SEARCH for now.");
      await sendButtons(from, "Start new search?", [{ id: "SEARCH", title: "SEARCH" }]);
      return;
    }
