```javascript
// index.js — Vivaho WhatsApp Matrimony Bot
      ...
      url,
      {
        ...
        interactive: {
          ...
          action: {
            ...
            sections: [{ title: trimTo(sectionTitle || "Select", 24), rows: rows.map((r) => ({ id: String(r.id).slice(0, 256), title: String(r.title).slice(0, 24) })) }]
          }
        }
      },
      { headers: { Authorization: "Bearer " + WHATSAPP_TOKEN, "Content-Type": "application/json" }, timeout: 20000 }
...
async function notifyAdminPayment(userPhone, profileId, planType) {
  ...
  if (planType === "PLAN_1_3MO") {
    planMsg = "Rs.300 for 3 months (Interest Only)";
    buttons = [{ id: "ADMIN_APPROVE_1_3MO_" + profileId + "_" + userPhone, title: "APPROVE Rs.300" }, { id: "ADMIN_REJECT_" + profileId + "_" + userPhone, title: "REJECT" }];
  } else if (planType === "PLAN_1_YEAR") {
    planMsg = "Rs.1000 for 1 year (Interest Only)";
    buttons = [{ id: "ADMIN_APPROVE_1_YEAR_" + profileId + "_" + userPhone, title: "APPROVE Rs.1000" }, { id: "ADMIN_REJECT_" + profileId + "_" + userPhone, title: "REJECT" }];
  } else if (planType === "PLAN_2_YEAR") {
    planMsg = "Rs.2000 for 1 year (Full Access)";
    buttons = [{ id: "ADMIN_APPROVE_2_YEAR_" + profileId + "_" + userPhone, title: "APPROVE Rs.2000" }, { id: "ADMIN_REJECT_" + profileId + "_" + userPhone, title: "REJECT" }];
  }
...
async function handleDirectCommand(from, cmd, args, temp, st) {
  ...
  if (cmd === "INTEREST") {
    ...
    const remaining = MAX_DAILY_INTEREST - (sentToday + 1);
  ...
  if (cmd === "ACCEPT" || cmd === "REJECT") {
    ...
    if (!existing || existing.status !== "SENT") { await sendText(from, "No pending interest found.\nकोई लंबित रिश्ता नहीं मिला।"); return; }
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // hi, hello, random chars -> Same as STOP
    ...
    if (effectiveInput === "VIEW_CONTACT") {
      ...
      if (!canViewContact) {
        await sendText(from, "*Premium Feature*\n*प्रीमियम सुविधा*\n\nView Contact requires Rs.2000 plan.\nसंपर्क देखने के लिए Rs.2000 प्लान आवश्यक है।");
        await sendButtons(from, "Choose option / एक विकल्प चुनें", [
