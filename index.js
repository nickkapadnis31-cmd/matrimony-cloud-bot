  ...
  try {
    await axios.post(
      url,
      {
        ...
        image: {
          link: imageLink,
          ...(caption ? { caption: trimTo(caption, 4096) } : {}),
        },
      },
      { headers: { Authorization: "Bearer " + WHATSAPP_TOKEN, "Content-Type": "application/json" }, timeout: 20000 }
    );
...
async function sendList(to, body, buttonText, rows, sectionTitle) {
  const phone = normalizePhone(to);
      ...
      url,
      {
        ...
        interactive: {
          ...
          action: {
            ...
            sections: [{
              title: trimTo(sectionTitle || "Select", 24),
              rows: rows.map((r) => ({ id: String(r.id).slice(0, 256), title: String(r.title).slice(0, 24) }))
            }]
          }
        }
      },
...
async function sendJoinSearchMenuButtons(to) {
...
async function getAllProfilesRows() {
  const sheets = await getSheetsClient();
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: PROFILE_TAB + "!A:AE" });
...
function profileRowToObj(row, rowIndex1Based) {
  return {
    ...
    favorites: row?.[24] || "",
    daily_interests: row?.[25] || "0",
    last_interest_date: row?.[26] || "",
    referred_by: row?.[27] || "",
    referral_code: row?.[28] || "",
    voice_intro_url: row?.[29] || "",
    viewed_profiles: row?.[30] || "",
  };
}
...
async function createProfile(phone, temp) {
  ...
  const row = [
    profile_id, phone, temp.name || "", "", temp.gender || "",
  ...
  await sheets.spreadsheets.values.append({ spreadsheetId: SHEET_ID, range: PROFILE_TAB + "!A:AE", valueInputOption: "RAW", requestBody: { values: [row] } });
  return profile_id;
}
...
async function appendRequest({ from_profile_id, to_profile_id, status, type, viewer_phone }) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "RAW",
    requestBody: { values: [[req_id, from_profile_id, to_profile_id, status, nowISO(), type, viewer_phone]] }
  });
}
...
async function notifyAdminPayment(userPhone, profileId, planType) {
  if (!ADMIN_PHONE) return;
  let planMsg = "", buttons = [];
  if (planType === "PLAN13MO") {
    planMsg = "₹99 for 1 month";
    buttons = [{ id: "ADMINAPPROVE13MO_" + profileId + "_" + userPhone, title: "APPROVE ₹99" }, { id: "ADMINREJECT_" + profileId + "_" + userPhone, title: "REJECT" }];
  } else if (planType === "PLAN1YEAR") {
    planMsg = "₹249 for 3 months";
    buttons = [{ id: "ADMINAPPROVE1YEAR_" + profileId + "_" + userPhone, title: "APPROVE ₹249" }, { id: "ADMINREJECT_" + profileId + "_" + userPhone, title: "REJECT" }];
  } else if (planType === "PLAN2YEAR") {
    planMsg = "₹499 for 1 year";
    buttons = [{ id: "ADMINAPPROVE2YEAR_" + profileId + "_" + userPhone, title: "APPROVE ₹499" }, { id: "ADMINREJECT_" + profileId + "_" + userPhone, title: "REJECT" }];
  }
...
// ===================== Search Helpers =====================
function getViewedProfiles(user) {
  return String(user.viewed_profiles || "").split(",").filter(Boolean);
}

function resetDailyInterestIfNeeded(user) {
  const today = new Date().toISOString().slice(0, 10);
  if (user.last_interest_date !== today) {
    user.daily_interests = 0;
...
async function showProfileCard(to, profile, temp) {
  ...
  if (profile.photo_url) await sendImageByLink(to, profile.photo_url, msg);
  else await sendText(to, msg);

  const viewed = getViewedProfiles(temp.userProfile || profile);
  if (!viewed.includes(profile.profile_id)) viewed.push(profile.profile_id);
  temp.userProfile = temp.userProfile || {};
  temp.userProfile.viewed_profiles = viewed.join(",");
...
async function handleDirectCommand(from, cmd, args, temp, st) {
  if (cmd === "MENU") {
    await sendButtons(from, "*Vivaho Menu*", [
      { id: "SEARCH", title: "SEARCH" },
  ...
  if (cmd === "SUCCESS") {
    await sendText(from, "Hundreds of people are finding their perfect life partner through Vivaho every month. Join them today!");
    await sendJoinSearchMenuButtons(from);
    return;
  }

  if (cmd === "SEARCHID") {
    const profileId = normalizeProfileId(args[0]);
    if (!profileId) { await sendText(from, "Usage: SEARCHID MH-XXXX"); return; }
    ...
    const msg = PROFILE_CARD_TEMPLATE(target, age);
    if (target.photo_url) {
  ...
  if (cmd === "MYPROFILES") {
    ...
    await sendText(from, msg);
    await sendButtons(from, "Select profile:", profiles.slice(0, 3).map(p => ({ id: "MYPROFILE_" + p.profile_id, title: p.profile_id })));
    return;
  }

  if (cmd === "DELETE") {
    const profileId = normalizeProfileId(args[0]);
    if (!profileId) { await sendText(from, "Usage: DELETE MH-XXXX"); return; }
  ...
  if (cmd === "DETAILS") {
    const profileId = normalizeProfileId(args[0]);
    if (!profileId) { await sendText(from, "Usage: DETAILS MH-XXXX"); return; }
    const target = await findProfileById(profileId);
    if (!target) { await sendText(from, "Profile not found."); return; }
    const age = calcAgeFromDobDDMMYYYY(target.date_of_birth);
  ...
  if (cmd === "INTEREST") {
    ...
    if (!canSendInterest) {
      ...
      const dailyKey = userProfile.profile_id + "_" + today;
      const sentToday = dailyInterestCount.get(dailyKey) || 0;
      if (sentToday >= MAX_DAILY_INTEREST) {
        await sendText(from, "*Daily limit reached!*\nYou've sent " + MAX_DAILY_INTEREST + " interests today. Come back tomorrow!");
        await sendJoinSearchMenuButtons(from);
        return;
      }
      dailyInterestCount.set(dailyKey, sentToday + 1);
    }
  ...
  if (cmd === "ACCEPT" || cmd === "REJECT") {
    ...
    if (!interestedProfileId) { await sendText(from, "Usage: ACCEPT MH-XXXX or REJECT MH-XXXX"); return; }

    const receiverProfiles = await findProfilesByPhone(from);
    if (!receiverProfiles.length) { await sendText(from, "No profile found. Please JOIN first."); return; }
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    else if (interactiveId === "SUCCESS") effectiveInput = "SUCCESS";
    else if (interactiveId === "SAVE_FAVORITE") effectiveInput = "SAVE_FAVORITE";
    ...
    if (cmd === "JOIN") {
      ...
      if (existing.length >= MAX_PROFILES_PER_PHONE) {
        ...
        await sendText(from, "You already have a profile!\n\nID: " + profile.profile_id + "\nStatus: " + status);
        await sendButtons(from, "Choose option", [
          { id: "MYPROFILES", title: "MY PROFILES" },
          { id: "SEARCH", title: "SEARCH" },
          { id: "MENU", title: "MENU" },
    ...
    if (cmd === "MENU") {
      await handleDirectCommand(from, "MENU", [], temp, st);
      return;
    }
    ...
    if (cmd === "SUCCESS") { await handleDirectCommand(from, "SUCCESS", [], temp, st); return; }
    ...
    if (effectiveInput === "SEARCH_BRIDE" || effectiveInput === "SEARCH_GROOM") {
      ...
      try {
        ...
        if (allVisible.length === 0) {
          await sendText(from, "No profiles in database. Be the first to JOIN!");
          await sendButtons(from, "Choose option", [
            { id: "JOIN", title: "JOIN" },
        ...
        const userProfile = (await findProfilesByPhone(from))[0];
        if (userProfile) {
          results.sort((a, b) => {
            let scoreA = 0, scoreB = 0;
            if (a.native_place === userProfile.native_place) scoreA += 20;
            if (b.native_place === userProfile.native_place) scoreB += 20;
            if (a.caste === userProfile.caste) scoreA += 10;
            if (b.caste === userProfile.caste) scoreB += 10;
            const ageA = calcAgeFromDobDDMMYYYY(a.date_of_birth);
            const ageB = calcAgeFromDobDDMMYYYY(b.date_of_birth);
            const userAge = calcAgeFromDobDDMMYYYY(userProfile.date_of_birth);
            if (userAge && ageA && Math.abs(ageA - userAge) <= 2) scoreA += 20;
            if (userAge && ageB && Math.abs(ageB - userAge) <= 2) scoreB += 20;
    ...
    if (effectiveInput === "SELECT_ACTION") {
      if (!temp.currentViewingProfile) {
        await sendText(from, "No profile selected. Starting new search...");
        await sendText(from, SEARCH_GENDER_MSG);
        await setState(from, "SEARCH_BRIDE_GROOM", {});
        await sendButtons(from, "Select", [
          { id: "SEARCH_BRIDE", title: "BRIDE" },
      ...
      buttons.push({ id: "SAVE_FAVORITE", title: "SAVE" });
    ...
    if (effectiveInput === "SAVE_FAVORITE") {
      if (!temp.currentViewingProfile) return;
      const profileId = temp.currentViewingProfile.profile_id;
      temp.favorites = temp.favorites || [];
      if (!temp.favorites.includes(profileId)) {
        temp.favorites.push(profileId);
        await sendText(from, "Profile " + profileId + " saved to favorites!");
      } else {
        await sendText(from, "Profile already in favorites.");
      }
      return;
    }
    ...
    if (st.step === "FILTER_WORK_CITY") {
      ...
      await sendList(from, "*Income*\n\nSelect income range", "Select", [
        { id: "INC_0_50K", title: "Rs.0 - 50,000" },
        { id: "INC_50K_1L", title: "Rs.50K - 1 Lakh" },
        { id: "INC_1L_3L", title: "Rs.1L - 3 Lakh" },
        { id: "INC_ABOVE_3L", title: "Above Rs.3 Lakh" },
    ...
    if (effectiveInput === "CLEAR_FILTERS") {
      ...
      await sendText(from, "Filters cleared. Starting fresh search...");
      await sendText(from, SEARCH_GENDER_MSG);
    ...
    if (effectiveInput === "MAKE_PAYMENT") {
      const userProfiles = await findProfilesByPhone(from);
      if (!userProfiles.length) {
        await sendText(from, "Please create profile first. Use JOIN to register.");
        await sendButtons(from, "Choose option", [
          { id: "JOIN", title: "JOIN" },
          { id: "MENU", title: "MENU" },
        ]);
      ...
      await sendButtons(from, "Select your plan", [
        { id: "PLAN_1_3MO", title: "Rs.300 (3mo)" },
        { id: "PLAN_1_YEAR", title: "Rs.1000 (1yr)" },
        { id: "PLAN_2_YEAR", title: "Rs.2000 (1yr)" },
      ]);
      return;
    }
    ...
    if (st.step === "PAYMENT_SELECT_PLAN") {
      if (effectiveInput === "PLAN_1_3MO" || effectiveInput === "PLAN_1_YEAR" || effectiveInput === "PLAN_2_YEAR") {
        ...
        let amount = effectiveInput === "PLAN_1_3MO" ? "Rs.300" : (effectiveInput === "PLAN_1_YEAR" ? "Rs.1000" : "Rs.2000");
        let planName = effectiveInput === "PLAN_1_3MO" ? "3 Months" : (effectiveInput === "PLAN_1_YEAR" ? "1 Year" : "1 Year Premium");
        ...
        if (QR_IMAGE_URL) {
          ...
          await sendText(from, "QR code not available. Please use UPI ID above.");
        }
    ...
    // ===================== REGISTRATION FLOW =====================
    if (st.step === "ASK_NAME") {
      if (!text || text.length < 2) { await sendText(from, "Please enter a valid name."); return; }
      temp.name = text;
      temp.surname = "";
    ...
    if (st.step === "ASK_GENDER") {
      ...
      if (!g) { await sendText(from, "Please select Male or Female."); return; }
      temp.gender = g;
    ...
    if (st.step === "ASK_DOB") {
      ...
      if (age === null || age < MIN_AGE) { await sendText(from, "Invalid date or age under " + MIN_AGE + ". Use DD-MM-YYYY format."); return; }
    ...
    if (st.step === "ASK_HEIGHT") {
      if (!text || text.length < 2) { await sendText(from, "Please enter height."); return; }
      temp.height = text;
      await setState(from, "ASK_RELIGION", temp);
      await sendText(from, "*Enter your religion*\n\nExamples: Hindu, Muslim, Christian");
      return;
    }

    if (st.step === "ASK_RELIGION") {
      if (!text || text.length < 2) { await sendText(from, "Please enter religion."); return; }
      temp.religion = text;
    ...
    if (st.step === "ASK_NATIVE_PLACE") {
      if (!text || text.length < 2) { await sendText(from, "Please enter native place."); return; }
      temp.native_place = text;
      await setState(from, "ASK_DISTRICT", temp);
      await sendText(from, "*Enter your district*");
      return;
    }

    if (st.step === "ASK_DISTRICT") {
      if (!text || text.length < 2) { await sendText(from, "Please enter district."); return; }
      temp.district = text;
    ...
    if (st.step === "ASK_EDU") {
      if (!text || text.length < 1) { await sendText(from, "Please enter education."); return; }
      temp.education = text;
    ...
    if (st.step === "ASK_JOB_TITLE") {
      if (!text || text.length < 2) { await sendText(from, "Please enter job title."); return; }
      temp.job_title = text;
      await setState(from, "ASK_INCOME", temp);
      await sendList(from, "*Monthly Income*", "Select", [
        { id: "INC_1", title: "Up to Rs.50,000" },
        { id: "INC_2", title: "Rs.50K - Rs.1 Lakh" },
        { id: "INC_3", title: "Rs.1L - Rs.3 Lakh" },
        { id: "INC_4", title: "Above Rs.3 Lakh" },
      ]);
      return;
    }
    ...
    if (st.step === "ASK_PHOTO") {
      if (msgType !== "image") { await sendText(from, "Please send a photo as image."); return; }
      ...
      try {
        ...
        if (!metaUrl) { await sendText(from, "Could not process photo."); return; }
    ...
    try {
      ...
      if (from) {
        await sendText(normalizePhone(from), "Something went wrong. Please try again.");
      }
    } catch (e) {
...
app.listen(PORT, () => {
  console.log("Vivaho Bot Running on Port " + PORT);
